...
namespace tensorrt_llm::kernels::tma {
  ...
  static inline PFN_cuTensorMapEncodeTiled_v12000 get_cuTensorMapEncodeTiled() {
    ...
    gpuErrchk(cudaGetDriverEntryPointByVersion("cuTensorMapEncodeTiled",
                                               &cuTensorMapEncodeTiled_ptr, 12000,
                                               cudaEnableDefault, &driver_status));

    if (driver_status != cudaDriverEntryPointSuccess) {
      std::cerr << "Could not get cuTensorMapEncodeTiled driver entry point"
                << std::endl;
      abort();
    }
  ...
  inline CUtensorMap createTensorMap(void *matrix_ptr,
                                     uint32_t matrix_height, uint32_t matrix_width,
